...
def gen_config(parameters: dict, product: bool):
    ...
    for num_layer in range(layer_min, layer_max + 1):
        for num_neuron in range(neuron_min, neuron_max + 1):
            for model_input in range(input_min, input_max + 1):
                ...
                file_name = (constants.POSITIVE_TEST_DIR + "/test" + str(num_layer) + str(num_neuron) +
                             str(model_input) + ".yaml")

                # Create the layer.
                a_layer = Layer(layertype= constants.TEST_CONFIG_LAYER_PARAMETERS["layer_type"],
                                numneurons=num_neuron,
                                activationtype= constants.TEST_CONFIG_LAYER_PARAMETERS["activation_type"],
                                dropout= constants.TEST_CONFIG_LAYER_PARAMETERS["dropout"])
                ...
                model_config = YAMLModel(modeltype= constants.TEST_CONFIG_MODEL_TYPE["model_type"],
                                         inputsize=model_input,
                                         numlayers=num_layer,
                                         layerlist=layer_data,
                                         hyperparameters = list(constants.TEST_CONFIG_MODEL_HYPERPARAMETERS),
                                         objective_fn= constants.TEST_CONFIG_MODEL_HYPERPARAMETERS["objective"],
                                         optimization_method= constants.TEST_CONFIG_MODEL_HYPERPARAMETERS["optimization"])
    ...
    if product:
        ...
        File test322p222.yaml has 3 layers with 2 neuron each layer
        ...
        for num_layer in range(layer_min, layer_max + 1):
            for num_neuron in range(neuron_min, neuron_max + 1):
                ...
                if num_layer > 1 and num_neuron > 1:
                    print("Product: layer:", num_layer, "neurons:", constants.TEST_CONFIG_HYPERPARAMETERS["neurons"])
                        ...
                        for model_input in range(input_min, input_max + 1):
                            ...
                            model_config = YAMLModel(modeltype= constants.TEST_CONFIG_MODEL_TYPE["model_type"],
                                                     ...
                                                     hyperparameters = list(
                                                         constants.TEST_CONFIG_MODEL_HYPERPARAMETERS),
                                                     objective_fn= constants.TEST_CONFIG_MODEL_HYPERPARAMETERS["objective"],
                                                     optimization_method= constants.TEST_CONFIG_MODEL_HYPERPARAMETERS["optimization"])
...
def dir_is_empty() -> bool:
    ...
    Returns:
        - <Bool> True if empty.
    """
    if os.listdir(constants.POSITIVE_TEST_DIR):
        return False
    else:
        return True
...
def get_config_dir() -> str:
    ...
    Returns:
         - CONFIG_DIR <string> Directory path where the YAML configuration files are stored
    """
    return constants.POSITIVE_TEST_DIR
...
def main():
    ...
    """
    # Write model yaml configuration files out to CONFIG_DIR/configuration
    gen_config(constants.TEST_CONFIG_HYPERPARAMETERS, product=True)
