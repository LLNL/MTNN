# !/usr/bin/env/ python
"""
Test script to run builder and train fully-connected neural network
with MNIST data using prolongation operators
"""

# standard examples packages
import os
import sys
import argparse
import time
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description = "Runs MTNN with a YAML configuration file")
...
def check_config(filepath: str) -> bool:
    """
    Check if configuration file can be read without errors, is not empty and is well-formed
    Args:
    ...
    try:
        if os.path.isfile(filepath):
            # TODO: Validate YAML configuration file. Use Python confuse API?
            return True
...
# Check if configuration file is NN kosher.
try:
...
# Check if configuration file is YAML kosher.
